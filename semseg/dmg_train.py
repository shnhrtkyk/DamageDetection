...
from criterion.loss  import  ce_loss, dice_loss, LovaszSoftmax, bce_loss
NUM_CHANNELS = 3
...
def get_loader(args):
    ...
    train_transform = MyTransform(reshape_size=None,crop_size=(256,256), augment=True)  # data transform for training set with data augmentation, including resize, crop, flip and so on
    val_transform = MyTransform(reshape_size=None,crop_size=(256,256), augment=False)   #data transform for validation set without data augmentation
...
def train(args, model):
    ...
    if args.cuda:
        ...
        criterion_dmg = LovaszSoftmax(weight_dmg).cuda()
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        model.train()
        count = 1
        for step, (images, labels,images_post,labels_post) in enumerate(loader):
            start_time = time.time()
            if args.cuda:
                images = images.cuda()
                labels = labels.cuda()
                images_post = images_post.cuda()
                labels_post = labels_post.cuda()

            inputs = Variable(images)
            targets = Variable(labels)
            inputs_post = Variable(images_post)
            targets_post = Variable(labels_post)

            outputs, outputs_post = model(inputs, inputs_post)
            #loss = criterion(outputs, targets[:, 0])
            #loss += dice_loss(targets, outputs)
            #loss += criterion_dmg(targets_post, outputs_post)
            loss = dice_loss(targets_post, outputs_post)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
